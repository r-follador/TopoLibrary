...
public class HGTWorker {
        ...
        for (int x=-1; x<=1; x++) {
            for (int y=-1; y<=1; y++) {
                if (!(x==0 && y == 0)) {
                    ...
                    gltfFile.addGLTFMesh(data, (lonCellNumber+6)/4, (latCellNumber+6)/4, cellWidth_LatMeters*4f, cellWidth_LonMeters*4f, false, offset_x, offset_y, 1f, 1f, uvTexture, uvTexture, true);

                } else {
                    short[][] data = load_3DEM(topLeftLatLon, lonCellNumber, latCellNumber, 0,0, hgtFileLoader);
                    gltfFile.addGLTFMesh(data, lonCellNumber, latCellNumber, cellWidth_LatMeters, cellWidth_LonMeters, true, 0, 0, 1f, 1f, new GLTFDatafile.UvTexture(true, false), uvTexture, true);
    ...
    public static GLTFDatafile getTileGLTF_3DEM(LatLonBoundingBox boundingBox, int zoomLevel, boolean enclosement, HGTFileLoader hgtFileLoader, String textureURL) throws IOException {
        ...
        for (int x=0; x<width; x++) {
            for (int y=0; y<height; y++) {
                ...
                GLTFDatafile.GLTFMesh mesh = gltfFile.addGLTFMesh(data, lonCellNumber, latCellNumber, cellWidth_LatMeters, cellWidth_LonMeters, enclosementNeeded, offset_x, offset_y, 1f, 1f, UVTexture, enclosementTexture, true);
    ...
    public static boolean writeHGT(File file, short[][] data, int cellsPerRow) throws IOException {
        OutputStream os = new FileOutputStream(file);
        ByteBuffer bb = ByteBuffer.allocate((cellsPerRow)*2);

        for (int y = 0; y < cellsPerRow; y++) {
            bb.clear();
            for (int x = 0; x < cellsPerRow; x++) {
                short height = data[x][y];
                bb.putShort(height);
            }
            os.write(bb.array());
        }
